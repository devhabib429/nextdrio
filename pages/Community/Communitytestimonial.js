...
const CommunityTestimonial = () => {
    ...
    return (
        <div className="p-4 md:p-14 bg-gray-100">
            ...
            >
                <div className="overflow-hidden">
                    <div className={`flex justify-start space-x-4 md:space-x-8 p-4 md:p-8 ${styles.scrollWrapper}`} style={{ animationPlayState: isPaused ? 'paused' : 'running' }}>
                        {duplicatedData.map((testimonial, index) => (
                            <div key={index} className={`bg-transparent border border-black rounded-lg p-4 flex-shrink-0 ${styles.card}`} style={{ maxWidth: 'calc(100% - 2rem)' }}>
                                <div className="w-16 h-16 rounded-full overflow-hidden mx-auto mb-4">
                                    <img src={testimonial.image} alt={testimonial.name} className="w-full h-full object-cover" />
                                </div>
                                <div className="flex items-start mb-2">
